...
# ─── Regenerate public/version.json (single remote source of truth) ──
# CI runs this script before both the Cloudflare deploy and the Android
# build, so the manifest always matches the freshly bumped release.
# minimumVersion* intentionally trails latest (v1 policy: never blocks).
cat > "public/version.json" << EOF
{
  "latestVersion": "$NEW_VERSION_NAME",
  "latestVersionCode": $NEW_VERSION_CODE,
  "minimumVersion": "1.0.300",
  "minimumVersionCode": 300,
  "playUrl": "https://play.google.com/store/apps/details?id=com.navron.chessduo",
  "notes": "Regenerated by scripts/bump-version.sh — do not edit by hand."
}
EOF
