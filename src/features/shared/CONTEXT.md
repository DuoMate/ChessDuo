...
## Recent Changes
- **2026-09-18**: Added `AI_COACH_FREE_DAILY_LIMIT = 3` + `AI_COACH_FREE_DAILY_LIMIT_ENABLED = true` — single source of truth for the AI Coach free daily allowance (purpose/current value/how-to-change documented on the constants; `COACH_TRIAL_WINDOW_MS` deprecated).
- **2026-09-18**: Added `DB_GAME_OVER_GRACE_MS` (4000) — bounded wait for the authoritative terminal broadcast before the DB-GAME_OVER fallback assumes peer abandonment.
