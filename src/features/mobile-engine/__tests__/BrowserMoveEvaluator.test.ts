...
describe('BrowserMoveEvaluator', () => {
  ...
  describe('constructor and initialization', () => {
    test('creates Worker eagerly in constructor', () => {
      new BrowserMoveEvaluator()
      expect(globalThis.Worker).toHaveBeenCalledWith('/stockfish/stockfish.js')
    })

    test('sends UCI init commands with MultiPV value 6 on construction', async () => {
      new BrowserMoveEvaluator()
  ...
  describe('error handling', () => {
    ...
    test('getInitError returns error when Worker creation fails in constructor', async () => {
    ...
    test('throws when evaluating before engine is ready', async () => {
      ...
      await expect(evaluator.evaluateMoves(['e2e4'], fen)).rejects.toThrow('not ready')
    })
  })
  ...
  describe('getBestScore', () => {
    test('returns the single top move and its score from MultiPV output', async () => {
      const evaluator = new BrowserMoveEvaluator()
      mockUciReady(mockWorker)
      await new Promise(r => setTimeout(r, 50))
