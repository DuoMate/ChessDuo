...
- **2026-08-03**: **R3 fix** — Added 15s engine-level timeout (`LOCK_TIMEOUT_MS`) to `waitForTeammateLock`. Previously, if the `player_locked` broadcast was lost, the promise hung forever (only M17 30s React guard compensated). Timeout cleaned up in `handleTeammateLocked` (normal path) and `handleTurnResolved` (R1 scenario — turn resolved before teammate locked). 12 regression tests added for R1/R2/R3 scenarios.
