...
- **2026-09-18**: Confirm-move support — `CoachGame` stages drops in `heldMove` when the `confirmMove` setting is on (the engine was never wired to the setting: drops submitted instantly, no bar). Confirm drains the hold into `applyPlayerMove`; Cancel clears it + remounts the board (`boardKey`, DuelGame semantics); held-move cancels first on web/native Back. Default stays OFF. Tests: `CoachGameConfirm.test.tsx` (stage/confirm/cancel/off). No engine/analysis/billing/game changes.
- **2026-09-18**: Calm best-move copy — `explainMove('best')` drops the repetitive "— excellent!" (`e4 is the engine's top choice.`). Only spoken superlative in coach TTS; voice features untouched. Tone locked by `coachAnalysis.test.ts` (no superlatives assertion).
