...
- `billing/` uses a `BillingProvider` interface abstraction — UI depends on `SubscriptionService`, never on the payment processor directly. Currently backed by Google Play Billing for Android. Web users directed to download the Android app. Architecture is ready for Apple In-App Purchases and other providers.
