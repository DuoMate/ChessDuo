...
## Recent Changes
- **2026-07-31**: **Bug 39 + Bug 40 + deep-link fixes** (merged to prod). Bug 39: `/?code=` invite links and room-code joins no longer read the RLS-locked `room_players` table before joining — `rooms.host_team` + the public `get_room_join_state` RPC drive team/fullness decisions in both the auto-join effect and `join_by_code`. Bug 40: Creem webhook now resolves empty event metadata via `creem.checkouts.retrieve(id)` and always logs + 200s; `verify-checkout` grants when the checkout OR its subscription is completed; `/premium` polls `getStatus()` after an unverified return. Also fixed: push game-invite "Session Expired" (ChallengePicker passed challenger id/WHITE — now friendId/BLACK), dead profile share link (`getProfileLink` → `/invite/[userId]`), challenge links not using the pre-created room, `chessduo:///` triple-slash deep-link parsing, `/replay` sign-in prompt, and `/invite` consent step.
