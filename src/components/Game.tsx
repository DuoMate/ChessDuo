...
import { setGameResult, GameSummary, TurnEntry } from '@/lib/resultsStore'
...
export function Game({ level, roomCode, mode, roomId, team, playerId: playerIdFromProps }: GameProps) {
  ...
  useEffect(() => {
    if (!isOnline && game && gameState.status === GameStatus.GAME_OVER) {
  ...
  }, [isOnline, game, gameState.status, level, router])
  ...
  const stats = !isOnline && game ? game.getStats() : null
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col overflow-hidden">
      ...
      {roomCode && (
        <div className="fixed top-16 left-0 right-0 z-40 p-3 bg-gray-800/90 backdrop-blur-sm border-b border-gray-700 text-center">
          <p className="text-gray-400 text-xs mb-1">Share this room code with your teammate:</p>
          <p className="text-xl font-bold text-yellow-400 tracking-widest font-mono">{roomCode}</p>
        </div>
      )}

      <main className={`flex-1 flex pt-16 pb-10 overflow-hidden ${roomCode ? 'mt-14' : ''}`}>
        <PlayerPanel
          team={Team.WHITE}
          capturedPieces={gameState.capturedByWhite}
          accuracy={stats?.player1Accuracy ?? 100}
      ...
      <StatsTicker
        syncRate={stats?.whiteSyncRate ?? 0}
        conflicts={stats?.whiteConflicts ?? 0}
