...
export function PageLoading({ label, className = '' }: PageLoadingProps) {
    ...
    setPhase((prev) => {
      switch (prev) {
        case 'idle': {
          ...
          const nextName = SEQUENCE[nextIdx % SEQUENCE.length]
  ...
  const currentName = SEQUENCE[sequenceIndex]
  const current = NODE_GRID[currentName]
  const px = current[0] * gridSpace
  const py = current[1] * gridSpace

  const targetName = sequenceIndex + 1 < SEQUENCE.length ? SEQUENCE[sequenceIndex + 1] : SEQUENCE[0]
