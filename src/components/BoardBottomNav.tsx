...
interface BoardBottomNavProps {
  ...
  /**
   * Tabs with no content in this surface (e.g. replay mode) render visibly
   * disabled instead of looking tappable-but-dead. Additive only — callers
...
function BoardBottomNavInner({ activeTab, onTabChange, onForward, onBackMove, onForwardMove, unreadChat, insightsLocked, disabledTabs = [] }: BoardBottomNavProps) {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-50 flex justify-center pb-[max(12px,env(safe-area-inset-bottom,12px))]"
    >
      <div className="flex items-center justify-around h-14 w-[95%] max-w-md px-1 rounded-2xl border border-slate-200/60 bg-white/90 shadow-[0_8px_32px_rgba(2,6,23,0.12)] backdrop-blur-2xl dark:border-slate-700/50 dark:bg-[var(--color-page-bg)]/90 dark:shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
        ...
        <button
          ...
          disabled={isDisabled('moves')}
          aria-disabled={isDisabled('moves')}
        ...
        <button
          ...
          aria-disabled={isDisabled('insights')}
          className={tabClass('insights', 'bg-blue-500/15 text-blue-300 shadow-[0_0_12px_rgba(59,130,246,0.2)]', `${inactiveTab} hover:bg-blue-500/5`)}
          aria-label="Insights"
        >
