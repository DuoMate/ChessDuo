...
## Recent Changes
- **2026-09-19**: **PiP auto-enter reliability.** `usePipEligibility` re-asserts eligibility on `App.appStateChange` resume (dynamic import mirroring `useCapacitorBackButton`, web-safe) via new `reaffirmPipEligible()` so a publish that raced plugin attach can never leave auto-enter disabled. Unmount still revokes.
...
- **2026-09-19**: **Supabase perf (ADR-007)** — `usePremium`: realtime `profiles UPDATE` effect no longer resubscribes on premium flips (`[]` deps + `isPremiumRef`); `useBadgeCount`: capped sender fetch (`limit 200`) + 5s cross-instance shared fetch (home + `(main)/layout` double-mount collapses to one network pair).
- **2026-09-20**: **Ads reliability ADS-03.** `PremiumProvider` discards any unused preloaded native ad when the user becomes premium (best-effort silent side effect only; entitlement logic untouched). Tests: `usePremiumAdDiscard.test.tsx`.
