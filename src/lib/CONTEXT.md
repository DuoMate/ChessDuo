...
## Recent Changes
- **2026-07-31**: **Bug 39 + Bug 40** fixes (deep links + premium activation). (1) `roomActions.ts` `createOnlineRoom` now inserts `host_team` on the room row and `matchmaking.ts` queue-created rooms set `host_team: 'WHITE'` — the joiner derives the opposite team WITHOUT reading the RLS-locked `room_players` table. `supabase.ts` types updated: `rooms.host_team` (`WHITE|BLACK|null`) and `challenge_links.room_id`. (2) `capacitorAuth.ts` fixed the triple-slash deep-link bug (`chessduo:///path` parsed as protocol-relative `//path`) by stripping leading slashes after the scheme, and added `lastHandledUrl` dedupe in `handleDeepLink` so `getLaunchUrl` + `appUrlOpen` don't double-handle. (3) `friends.ts` `getProfileLink()` now returns the `/invite/[userId]` URL — the public `/profile/[userId]` route does not exist, so sharing a profile previously produced a dead link. (4) `challenges.ts` pre-created duel rooms now set `host_team: 'WHITE'`; the acceptor joins the pre-created `room_id` instead of creating a second room.
