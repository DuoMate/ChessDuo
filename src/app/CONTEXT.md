...
- **2026-07-15**: Animation & overlay fixes — `useScrollLock` ref-counted for nested modals. Turn indicator in `BoardTopBar` now animates exit on turn change. `ChallengePicker` and `FriendsPanel` chat overlay backdrops get proper exit animations. Dead props removed from `GameOverModal`. `ResignConfirmModal` uses shared `MODAL_BACKDROP` constant.
