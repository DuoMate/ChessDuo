...
## Recent Changes
- **2026-08-01**: **Bug 41** fix — the page now re-runs `runLoad()` (status + verify-on-return) when the Capacitor app returns to the foreground after a checkout was opened (`appStateChange` resume, gated by a `checkoutPendingRef` set in `handleSubscribe`). On mobile the checkout opens in the external system browser and the deep-link hand-off back to the app is not guaranteed, so without this the user who paid never saw the "You're Premium!" success screen (web showed it because it's a same-tab redirect). Verification runs on the resume path too.
