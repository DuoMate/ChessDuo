...
## Recent Changes
- **2026-09-16**: **Browser premium flow fix (platform-check-first)** — `page.tsx` now determines `isNativeApp()` (existing `window.Capacitor.isNativePlatform()` detection) BEFORE any billing operation. Web skips `SubscriptionService.getPlans()` entirely (entitlement status only), so no plugin lookup, no ProductDetails query, and no spurious "plans could not be loaded" error; `handleSubscribe`/`handleRestore` early-return on web where the existing "Download on Google Play" CTA is the upgrade path. Android flow byte-identical.
...
- **2026-09-19**: **Cancel/error-state hardening + bottom-nav clearance** — `handleSubscribe` clears `errorDetail` on new attempts, explicitly clears `error`+`errorDetail` on `cancelled` (silent return, no banner), and guards the 30s safety net with a settled flag so it can't fire after a silent cancel. Page shell bottom padding now includes `max(12px,env(safe-area-inset-bottom,12px))` so scrolled-to-bottom content clears the fixed `HomeBottomNav` pill. Success/verify/restore/pricing logic unchanged.
