...
### 14. Quick Play — "Play My Move" (opt-in gameplay variant)

**RULE**: A **Quick Play-only**, opt-in variant, default **OFF**. It is strictly isolated and must never affect Duo, 4-Player, 1v1 (Duel), AI Coach, or Replay. When OFF, Quick Play behavior is byte-for-byte unchanged.

- **Setting**: `Settings.playMyMove` (`src/lib/settingsStorage.ts`, default `false`), toggled from the Home `ConfigurationPanel` via its `showPlayMyMove` prop (rendered only when `selectedGameMode === 'quick'`). Do NOT add it to the global `SettingsPanel`.
- **Snapshot (new-game boundary)**: read once at `LocalGame` construction (`Game.tsx` `new LocalGame(time, color, ... && getSetting('playMyMove'))`) and held immutably (`LocalGame._playMyMove`); mid-game changes affect only the NEXT game.
- **Resolution boundary**: `LocalGame.resolvePendingMoves()` — when `_playMyMove && currentTeam === humanTeam`, the human's legal move is forced as `winningMove` (`winnerId='player1'`), and the teammate bot's move becomes the shadow/hint via the existing `loserId`/`loserFrom`/`loserTo` → `ChessBoard` retraction + `MovePlayback`. The teammate checkmate short-circuit is skipped when forced. Opponent turns are never affected.
- **Bot engine is NOT disabled**: `teammateBot.selectBestMove` and `evaluateMoves` still run; difficulty/Elo/depth/UCI/evaluation are unchanged.
- **Replay/history** contain only the actual (player) move; the shadow is stored separately and never applied. No persistence/schema change.
- **Never broaden** this into shared game-engine/resolution logic; the effective guard is exactly `QUICK_PLAY && playMyMove === true` (additive `LocalGame` ctor param + private flag; no `GameInterface` change).
